...
function SpiralGalaxy({ scrollProgress }) {
  ...
  useFrame((_, delta) => {
    if (galaxyRef.current) {
      ...
      galaxyRef.current.position.y = THREE.MathUtils.lerp(galaxyRef.current.position.y, -1.8 + scrollProgress * 1.2, 0.05);
    }
  ...
  return (
    <group ref={galaxyRef} position={[0, -1.8, -1]} rotation={[0.55, 0, 0]}>
...
function SolarSystemPlanets({ scrollProgress }) {
  ...
  return (
    <group ref={groupRef} position={[0, -1.8, -1]} rotation={[0.55, 0, 0]}>
