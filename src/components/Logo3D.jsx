...
export default function Logo3D({ size = 'hero', className = '' }) {
  ...
  return (
    ...
    >
      {/* 3D Motion Container */}
      <div
        className="w-full h-full relative transform-gpu transition-transform duration-150 ease-out"
        style={{
      ...
      >
        ...
        <div
          className={`absolute -inset-4 sm:-inset-6 rounded-3xl blur-xl transition-all duration-500 opacity-70 group-hover:opacity-100 ${
            isNav
          ...
          style={{
            ...
            filter: isHovered ? 'blur(28px)' : 'blur(18px)',
          }}
        />
        ...
        {!isNav && (
          <div
            className="absolute -inset-6 sm:-inset-8 rounded-full border border-cyan-500/30 animate-spin-slow pointer-events-none"
            style={{ transform: 'translateZ(-15px)' }}
          >
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-2.5 h-2.5 rounded-full bg-cyan-400 shadow-[0_0_12px_#00f0ff]" />
            <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-2.5 h-2.5 rounded-full bg-purple-400 shadow-[0_0_12px_#a855f7]" />
            <div className="absolute left-0 top-1/2 -translate-y-1/2 w-2.5 h-2.5 rounded-full bg-pink-400 shadow-[0_0_12px_#ec4899]" />
            <div className="absolute right-0 top-1/2 -translate-y-1/2 w-2.5 h-2.5 rounded-full bg-cyan-400 shadow-[0_0_12px_#00f0ff]" />
          </div>
        )}

        {/* Layer 3: Glass HUD Container */}
        <div
          className={`w-full h-full rounded-2xl sm:rounded-3xl p-1.5 sm:p-2 glass-panel relative overflow-hidden flex items-center justify-center border-2 transition-all duration-300 ${
            isHovered
              ? 'border-cyan-400 shadow-[0_0_40px_rgba(0,240,255,0.6)]'
              : 'border-cyan-500/40 shadow-[0_0_20px_rgba(0,240,255,0.25)]'
          }`}
          style={{ transform: 'translateZ(15px)' }}
        >
          ...
          <div
            ...
            style={{
              background: `radial-gradient(circle at ${glarePos.x}% ${glarePos.y}%, rgba(255, 255, 255, 0.45) 0%, rgba(0, 240, 255, 0.2) 35%, transparent 75%)`,
            }}
          />
          ...
          <div
            ...
            style={{
              ...
              animation: isHovered ? 'shimmer 2.2s infinite' : 'none',
            }}
          />
          ...
          {/* Corner Target Reticles (Hero Mode) */}
          {!isNav && (
            <>
              <div className="absolute top-1.5 left-1.5 w-3 h-3 border-t-2 border-l-2 border-cyan-400 group-hover:w-4 group-hover:h-4 transition-all duration-300 pointer-events-none" />
              <div className="absolute top-1.5 right-1.5 w-3 h-3 border-t-2 border-r-2 border-cyan-400 group-hover:w-4 group-hover:h-4 transition-all duration-300 pointer-events-none" />
              <div className="absolute bottom-1.5 left-1.5 w-3 h-3 border-b-2 border-l-2 border-purple-400 group-hover:w-4 group-hover:h-4 transition-all duration-300 pointer-events-none" />
              <div className="absolute bottom-1.5 right-1.5 w-3 h-3 border-b-2 border-r-2 border-purple-400 group-hover:w-4 group-hover:h-4 transition-all duration-300 pointer-events-none" />
            </>
          )}
        </div>
      </div>
